...
const FeatureCard: React.FC<FeatureCardProps> = ({ feature, index }) => {
  ...
  return (
    <div
      ref={ref}
      className="relative flex w-full flex-col overflow-hidden py-16"
      style={{
        background: feature.background,
        zIndex: index, // higher index → painted above previous
      }}
    >
      {/* Subtle Grid & Noise Layers */}
...
const FeaturesSection: React.FC = () => {
  return (
    <section
      className="relative"
      style={{
        height: `${features.length * 100}vh`, // total height = number_of_features × 100vh
      }}
    >
      {features.map((feature, index) => (
        <div
          ...
          style={{
            zIndex: index, // so later features paint on top of earlier ones
          }}
        >
