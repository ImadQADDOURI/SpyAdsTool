...
const PageNameWithPopover: React.FC<PageNameWithPopoverProps> = memo(
  ({ snapshot }) => {
    const {
      page_name,
      page_categories,
    ...
    }, [page_categories, link_url]);

    // 🛡️ Early return if no essential data
    if (!page_name && !page_profile_picture_url) {
      return (
...
PageNameWithPopover.displayName = "PageNameWithPopover";
