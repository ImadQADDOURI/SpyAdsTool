...
const AffiliateMarketingCalculator: React.FC = () => {
  ...
  const [affiliateCommissionRate, setAffiliateCommissionRate] = useState([10]); // Default 10%
  ...
  const totalAffiliateCommissions = useMemo(() => {
    return (affiliateCommissionRate[0] / 100) * totalRevenue;
  }, [affiliateCommissionRate, totalRevenue]);
  ...
  const netRevenue = useMemo(() => {
    return totalRevenue - (totalRefundCost + totalAffiliateCommissions);
  }, [totalRevenue, totalRefundCost, totalAffiliateCommissions]);
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="overflow-hidden border-0 bg-white/80 shadow-xl backdrop-blur-sm transition-all duration-300 hover:shadow-2xl dark:bg-gray-900/80">
          ...
          <CardContent className="space-y-6 p-6">
            ...
            {/* Affiliate and Refund Metrics */}
            <div className="grid gap-4 md:grid-cols-2">
              <SliderInput
                label="Affiliate Commission Rate"
                value={affiliateCommissionRate}
                onValueChange={setAffiliateCommissionRate}
                icon={<Users size={20} />}
              />
              <SliderInput
                label="Refund Rate"
            ...
            <Card className="overflow-hidden border-0 bg-white/80 shadow-xl backdrop-blur-sm dark:bg-gray-900/80">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 bg-gradient-to-r from-[#6566F1] to-[#B977F8] bg-clip-text text-transparent">
                  <Users size={20} className="text-[#6566F1]" />
                  Affiliate Commissions
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-3xl font-bold text-gray-900 dark:text-white">
                  ${totalAffiliateCommissions.toFixed(2)}
                </p>
            ...
            <CardContent>
              <div className="h-[350px]">
                <CalculatorPieChart
                  ...
                  netProfit={netProfit}
                  totalSpending={
                    adSpend + totalExtraCharges + totalAffiliateCommissions
                  }
